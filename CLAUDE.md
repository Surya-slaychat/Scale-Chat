...
| Device Contacts Sync | 🚧 partial | 🚧 partial | PR 6.1 + 6.2 shipped — shared schemas, expanded `toE164India`, and `POST /contacts/discover` (stateless, rate-limited 10/min, strict privacy contract — never leaks `userId`). PR 6.3 (bulk endpoint) + 6.4 (frontend `expo-contacts` + Import Contacts modal) pending. See `docs/progress/device-contacts.md`. |
