...
| Polls (1-on-1) | 🚫 queued (2.F-front) | ✅ (2.F-back) | New `PollsModule`: create poll → server-authored POLL message + `poll_messages`/`poll_options` rows in one tx; vote endpoint single-select replaces / multi-select diffs under `pg_advisory_xact_lock(pollMessageId)`; close is sender-only. `MessageDto.poll: PollAggregateSchema \| null` folded in via batched `injectPolls`. New `poll:voted` socket event personalised per viewer via the new `user:{userId}` room (joined on connect; will carry 2.H call-ring fan-out). 7 e2e cases green; mobile composer + bubble queued for PR-2. |
